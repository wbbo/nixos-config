...
if [ -x "$MIHOMO_DIR/mihomo" ]; then
  ...
  if [ "${export_use_mihomo:-0}" = 1 ]; then
    ...
    # TUN 透明代理需要本机 DNS 被 mihomo 接管 (fake-ip 入口): NM 的 rc-manager=
    # resolvconf 模式下 /etc/resolv.conf 是独立文件, 直接写入。与生产 resolv.conf
    # (mihomo.nix) 同构: 全写公网 DNS —— 查询出网即被 TUN dns-hijack any:53
    # 截回 mihomo 走 fake-ip, 不需要 127.0.0.1; MAXNS=3 满额全被劫持, 不存在
    # 混入网关 DNS 的污染路径 (网关/内网 DNS 因 route-exclude 绕过劫持, 会
    # 拿到污染应答且 mihomo 无域名元数据救不回, 绝不可入列)。NM 重新生成
    # (link 变化/DHCP 续约) 会覆盖回网关 DNS —— 失败仅告警, 此时 fake-ip
    # 断供, 由下方环境变量兜底 (nix 拉取确定可用); 面板切节点后可重试
    if printf 'nameserver 1.1.1.1\nnameserver 8.8.8.8\nnameserver 9.9.9.9\n' > /etc/resolv.conf 2>/dev/null \
       && grep -q '1.1.1.1' /etc/resolv.conf; then
      info "已设置本机 DNS: 1.1.1.1/8.8.8.8/9.9.9.9 (出网即被 TUN dns-hijack 截回 mihomo)"
    else
      warn "无法写入 /etc/resolv.conf, 本机 DNS 保持网关 -> 污染风险 (环境变量兜底不受影响)"
    fi
