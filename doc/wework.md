...
### 8.11 "白条纹"的真凶: niri 的 focus ring (2026-09-18 收尾)

排查了一大圈 (外框窗、幽灵帧、shm、XShape) 之后, 用**同样的坐标对比"聚焦/未聚焦"
两张截图**定位到: 应用窗左边缘那条青绿色竖线 (实测 `rgb(118,173,159)`, 宽 ~3px,
未聚焦时为 `rgb(26,23,20)` 即不存在) **就是 niri 画在聚焦窗上的 focus ring**,
正对上本仓库 `config.kdl` 里的 `active-gradient from="#88d6bbcc" to="#a8cbe2cc"`。

**不是 bug, 是配置**。用户当时说"失去焦点也还在", 实际是因为要检查就得点一下,
一点焦点又回到该窗 —— 这类"观察行为本身改变了被观察状态"的坑值得记一笔:
**判断某个装饰是否随焦点变化, 必须由脚本切焦点 + 抓图对比, 不能靠手点。**

副产物: 为排查它做的那些改动里, `unmap → XShape` 是**不必要的** (XShape 挡不住
Xwayland 呈递的 buffer, 而且"幽灵帧"从未被证实), 已改回 unmap。
