...
不透明黑渲染**, 完整取证见 **§八 根因定位**。已提交 issue:
**https://github.com/Supreeeme/xwayland-satellite/issues/502** —— 标题
"NVIDIA: 32-bit ARGB child windows are never captured — window stays
permanently black (not just first-frame)"; 该 issue 的定位 (satellite 捕获失败)
经实机取证证伪, 更正内容见 §八。上游修复后 wework-fix 守护可退役。
...
### 8.12 右键菜单"四角黑块"的真凶: 应用自己画的 (2026-09-18 收尾)

**现象**: 右键菜单是圆角矩形, 四个角外面露出几个黑像素 (用户从最初就报过,
一直以为是黑窗问题的残留)。

**取证**: 等菜单弹出时, 同时抓"菜单窗的 X 侧原始像素"和"屏幕同坐标像素"
(探针 `menucorner.py`)。结果:

```
菜单窗 159x307 **depth=24**          ← 没有 alpha 通道
角 (2,2)   = (0,0,0)   角 (156,2)  = (0,0,0)
角 (2,304) = (0,0,0)   中心(79,153) = (255,255,255)
```

**黑像素就在应用自己的窗口内容里** —— 不是合成器加的, 也不是 satellite 加的。

**根因**: 菜单窗是 depth-24, 圆角外侧没法透明, 应用只能涂黑。
(它给"外框窗"用的是 depth-32 带 alpha 的窗, 给菜单却是 depth-24 —— 应用自己的
实现取舍。)

**归属**: 只能由 Wine/WeCom 侧改 (菜单窗改用带 alpha 的 visual)。satellite /
Xwayland / niri 都无能为力 —— 它们拿到什么显示什么。已连同主结论一起写进
issue #502 的更正稿。

**方法论收获**: 判"黑块是谁画的"最干净的办法就是**同坐标对比 X 侧像素与屏幕像素** ——
X 侧有 = 应用画的, 只有屏幕有 = 合成路径加的。探针脚本见
`~/.local/share/argb-repro/menucorner.py`。

---

## 九、排查方法论与归因教训 (2026-09-18 增补)

这一章不记"结论", 只记**怎么查**和**怎么避免得出错结论** —— 本议题从
"satellite 捕获失败"一路查到"niri 丢 alpha", 中间走错四次, 每次都值得记。

### 9.1 取证手法 (按好用程度排序)

| 手法 | 用途 | 本次实例 |
|------|------|----------|
| **同坐标对比「X 侧像素」与「屏幕像素」** | 判"某个像素是谁画的": X 侧有 = 应用画的; 只有屏幕有 = 合成路径加的 | 一条命令定性了菜单四角黑块 (X 侧三个角就是 `(0,0,0)`) |
| **协议追踪** `WAYLAND_DEBUG=1` | 看 buffer 的**格式/尺寸**到底传的是什么 | 两跳都是 `AR24` → 排除 Xwayland 与 satellite |
| **A/B 只变一个变量, 且尺寸/聚焦都对齐** | 判"哪一层导致" | dmabuf 46% 新增黑 vs shm 9.9% → 定位到 niri |
| **导出窗口像素为 PNG 直接看** | 比任何数值统计都直观 | 一眼看出"透明底 + 一圈白色圆角边" |
| 脚本**切焦点**+抓图对比 | 判"某装饰是否随焦点变化" | 焦点环 (手点会因为"点回去又聚焦"得出反结论) |

### 9.2 归因的四条教训 (按踩到的顺序)

1. **"渲染正常"≠"可用"**。shm (`-glamor none`) 下抓屏看窗口渲染完全正常、A/B 也好,
   结论"它就是解法" —— 但实际**鼠标点不动**。Xwayland 关掉 glamor 后 DRI3/Present 一并失效,
   这类"事件循环坏了"的问题只有**真的去点一下**才暴露。
   → **凡是会改变渲染路径的改动, 验收必须包含"实际交互"**, 不能只看抓屏。
2. **别去找"符合自己假设的那条线"**。用户说"白条纹", 我量到窗口最外缘一条青绿线、
   颜色对上 focus ring, 就宣布"真凶是焦点环" —— 而用户指的是**内容里面**那条白色竖线
   (ARGB 外框窗的边框)。两条线相距几像素。
   → **先问清"你说的是哪一条"**(位置/颜色/是否随某操作变化), 再动手。
3. **别用"更优雅的做法"替换已经验证有效的做法**。unmap 已验证能藏住外框窗, 我出于
   "避免幽灵帧"的**推测**换成 XShape —— 结果 XShape 挡不住 Xwayland 呈递的 buffer,
   白线又回来了。**推测出来的缺陷 + 未实测的新方案 = 倒退**。
4. **一条线索的相关性不等于因果**。"点击失效"与"unmap 开关"看似相关 (改回去就好了),
   实际真凶是**同时跑着两个实例**。相关性能把人锁死在错误的方向上 —— 越想越"证实"。
   → **证伪比证实重要**: 主动找一个能推翻自己假设的实验 (这里就是"全新实例 + 同样配置")。

### 9.3 与上游打交道

- **报 issue 前先把责任层定准**。本次原 issue 把原因归给 xwayland-satellite, 实机取证后
  发现它连像素路径都没有 —— 更正评论的价值不如**直接给真正的责任方 (niri) 开一条**。
- **一条 issue 的价值 = 证据链的完整性**。可复现的最小步骤 + 协议追踪 + 控制变量的 A/B +
  "我排除了什么"的清单, 比"我这里坏了"有用得多。
- **GitHub token 必须 classic** (`ghp_`): 细粒度 (`github_pat_`) 对**非自有的公开仓库**
  最多只读, 提 issue/PR 会 403。详见 `CLAUDE.md` 的「GitHub token」一节。
